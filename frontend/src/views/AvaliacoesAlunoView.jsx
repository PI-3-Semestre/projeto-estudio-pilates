...
import { AvaliacaoProvider, useAvaliacao } from '../context/AvaliacaoContext';
...
const AvaliacoesAlunoContent = () => {
  ...
  // **SIMPLIFICADO**
  // Busca as avaliações diretamente com o ID do aluno da URL.
  useEffect(() => {
    if (alunoId) {
      fetchAvaliacoes(alunoId);
    }
  }, [alunoId, fetchAvaliacoes]);

  // O nome do aluno agora é pego da primeira avaliação da lista.
  const alunoNome = useMemo(() => avaliacoes[0]?.aluno_nome || 'Aluno', [avaliacoes]);
  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      if (isEditing) {
        // A atualização não precisa do alunoId, mas o refresh da lista sim.
        await atualizarAvaliacao(avaliacaoSelecionada.id, formData, alunoId);
      } else {
        // A criação aqui usa a rota global, que precisa do ID do aluno.
        await criarAvaliacao(alunoId, formData);
      }
      setIsCreating(false);
      setIsEditing(false);
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    if (avaliacaoSelecionada && window.confirm('Tem certeza que deseja excluir esta avaliação?')) {
      try {
        await deletarAvaliacao(avaliacaoSelecionada.id, alunoId);
      } catch (err) {
    ...
    if (avaliacoes.length === 0) {
        return (
            <div className="text-center py-10">
                ...
                <button onClick={handleStartCreating} className="flex mx-auto min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-xl h-12 px-5 bg-action-primary text-text-light gap-2 text-base font-bold leading-normal tracking-[0.015em]">
...
const AvaliacoesAlunoView = () => (
  <AvaliacaoProvider>
    <AvaliacoesAlunoContent />
  </AvaliacaoProvider>
);

export default AvaliacoesAlunoView;
