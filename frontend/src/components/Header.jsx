        ...
        <>
            <header className="flex items-center bg-card-light dark:bg-card-dark p-4 pb-2 justify-between sticky top-0 z-30 shadow-sm">
                <div className="flex w-12 shrink-0 items-center justify-start">
                    {showBackButton ? (
                        <button onClick={handleBack} className="p-2 text-text-light dark:text-text-dark">
                            <Icon name="arrow_back" style={{ fontSize: '28px' }} />
                        </button>
                    ) : (
                        <button onClick={toggleMenu} className="p-2 text-text-light dark:text-text-dark">
                ...
                <div className="flex w-auto items-center justify-end gap-1">
                    <button onClick={toggleTheme} className="flex h-10 w-10 cursor-pointer items-center justify-center overflow-hidden rounded-full bg-transparent text-text-light dark:text-text-dark">
                        <Icon name={theme === 'light' ? 'dark_mode' : 'light_mode'} style={{ fontSize: '24px' }} />
                    </button>
                    
                    <div className="relative" ref={notificationsRef}>
                        <button onClick={toggleNotifications} className="relative flex h-10 w-10 cursor-pointer items-center justify-center overflow-hidden rounded-full bg-transparent text-text-light dark:text-text-dark">
            ...
            {isMenuOpen && (
                <>
                    <div className="fixed inset-0 z-30 bg-black/50" onClick={toggleMenu}></div>
