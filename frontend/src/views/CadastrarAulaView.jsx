...
import useCadastrarAulaViewModel from "../viewmodels/useCadastrarAulaViewModel";
import { useToast } from "../context/ToastContext";

const CadastrarAulaView = () => {
  ...
  const handleFormSubmit = (e) => {
    ...
    if (!formData.data_hora_inicio) {
      showToast("Por favor, preencha a data e a hora.", "error");
      return;
    }
  ...
  const confirmCreateAula = async () => {
    ...
    try {
      ...
      // A navegação já é tratada no viewmodel
    } catch (err) {
      const errorMessage =
  ...
  if (!isStaff) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-background-light dark:bg-background-dark">
        <p className="text-red-500">{error || "Você não tem permissão para acessar esta página."}</p>
      </div>
    );
  }
...
export default CadastrarAulaView;
